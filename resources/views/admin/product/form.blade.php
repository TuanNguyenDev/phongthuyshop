...
<div class="col-sm-12">
	...
	<form action="{{route('product.save')}}" method="post" accept-charset="utf-8" novalidate enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<input value="{{old('ten_san_pham', $model->ten_san_pham)}}" type="text" id="ten_san_pham" name="ten_san_pham" class="form-control" placeholder="Tên sản phẩm">
		</div>
		{{-- <div class="form-group">
				<label for="product_url">Slug Url</label>
				<input id="product_url" type="text" value="{{old('slug', $modelSlug->slug)}}" name="slug" class="form-control" placeholder="Slug url">
				@if (count($errors) > 0)
		...
		<div class="form-group">
			...
			<input type="text" id="gia_nhap" name="gia_nhap" value="{{old('gia_nhap',$model->gia_nhap)}}" class="form-control" placeholder="Giá nhập">
			@if (count($errors) > 0)
				<span class="text-danger">{{$errors->first('gia_nhap')}}</span>
			@endif
		</div>
		<div class="form-group">
